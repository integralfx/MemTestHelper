...
Global $hwnd_gui                            
Global $btn_auto_ram                        ; button to automatically input free RAM
...
Func create_main_tab()
    ...
    $btn_auto_ram = GUICtrlCreateButton("RAM to test (MB):", 10, 25, 100, 20)
    GUICtrlSetOnEvent($btn_auto_ram, "btn_auto_ram_clicked")
    GUICtrlSetTip($btn_auto_ram, "Automatically input free RAM")
    $edt_ram = GUICtrlCreateEdit("", 135, 25, 50, 20, 0)
    
    GUICtrlCreateLabel("Number of threads:", 10, 55)
    $cbo_threads = GUICtrlCreateCombo("", 135, 50, 50, 20, $CBS_DROPDOWNLIST)
    GUICtrlSetData($cbo_threads, get_cbo_threads(), $MAX_THREADS)
    GUICtrlSetOnEvent($cbo_threads, "cbo_threads_selected")
    
    $btn_run = GUICtrlCreateButton("Run", 10, 75, 80, 30)
    GUICtrlSetOnEvent($btn_run, "run_memtest")
    
    $btn_stop = GUICtrlCreateButton("Stop", 110, 75, 80, 30)
    GUICtrlSetState($btn_stop, $GUI_DISABLE)
...
Func validate_input()
    ...
    If $amount / $threads > $MEMTEST_MAX_RAM Then
